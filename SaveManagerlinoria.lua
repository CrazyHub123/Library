...
local SaveManager = {} do
	...
	function SaveManager:BuildConfigSection(tab)
      ...
      if isfile(self.Folder .. '/settings/autoload.txt') then
			local name = readfile(self.Folder .. '/settings/autoload.txt')
			getgenv().Linoria.Options.SaveManager_ConfigList:SetValue(name)
		end
		...
		getgenv().AutoOverwrite = false

		section:AddToggle('__AutoStoreFruit', {
    		Default = false,
    		Text = 'Auto Overwrite config',
    		Tooltip = 'auto overwrite config every 2s',
    		Callback = function(s)
        		getgenv().AutoOverwrite = s
    		end
		})

		spawn(function()
   		  while task.wait(2) do
        	if getgenv().AutoOverwrite then
            	pcall(function()
					local name = getgenv().Linoria.Options.SaveManager_ConfigList.Value
